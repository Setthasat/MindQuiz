...
class SubmissionAPI {
    constructor() {
        this.submitQuiz = (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(0, auth_utils_1.isValidString)(roomId, 1, 50) || !(0, auth_utils_1.isValidString)(userId, 1, 50) || !Array.isArray(answers)) {
            ...
            try {
                ...
                const existingSubmission = yield submission_model_1.Submission.findOne({ roomId, userId });
                if (existingSubmission) {
                    baseResponseInst.setValue(400, "User has already submitted this quiz", null);
                    return res.status(400).json(baseResponseInst.buildResponse());
                }
                const processedAnswers = answers.map((userAnswer, index) => {
        ...
        this.gradeWritingQuestion = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const baseResponseInst = new baseresponse_utils_1.BaseResponse();
            const { submissionId, questionIndex, score, feedback, ownerId } = req.body;
            if (!(0, auth_utils_1.isValidString)(submissionId, 1, 50) || typeof questionIndex !== 'number' || typeof score !== 'number' || score < 0) {
                baseResponseInst.setValue(400, "Invalid grading data", null);
                return res.status(400).json(baseResponseInst.buildResponse());
            }
            try {
                const submission = yield submission_model_1.Submission.findOne({ submissionId });
                if (!submission) {
                    baseResponseInst.setValue(404, "Submission not found", null);
                    return res.status(404).json(baseResponseInst.buildResponse());
                }
                const room = yield room_model_1.Room.findOne({ roomId: submission.roomId });
                if (!room) {
                    baseResponseInst.setValue(404, "Room not found", null);
                    return res.status(404).json(baseResponseInst.buildResponse());
                }
                ...
                const allWritingQuestionsGraded = writingQuestionIndices.every(index => { var _a; return (_a = submission.answers.find(a => a.questionIndex === index)) === null || _a === void 0 ? void 0 : _a.gradedAt; });
        ...
        this.getSubmissions = (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(0, auth_utils_1.isValidString)(roomId, 1, 50) || !(0, auth_utils_1.isValidString)(ownerId, 1, 50)) {
            ...
            try {
                ...
                const submissions = yield submission_model_1.Submission.find({ roomId }).sort({ submittedAt: -1 });
                const submissionData = submissions.map(sub => ({
                    ...
                    needsGrading: sub.answers.filter(a => room.question[a.questionIndex].answerType === 'writing' && !a.gradedAt).length
                }));
        ...
        this.getSubmissionDetail = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const baseResponseInst = new baseresponse_utils_1.BaseResponse();
            const { submissionId, ownerId } = req.body;
            if (!(0, auth_utils_1.isValidString)(submissionId, 1, 50) || !(0, auth_utils_1.isValidString)(ownerId, 1, 50)) {
                baseResponseInst.setValue(400, "Invalid parameters", null);
                return res.status(400).json(baseResponseInst.buildResponse());
            }
            try {
        ...
        this.getUserSubmission = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const baseResponseInst = new baseresponse_utils_1.BaseResponse();
            const { roomId, userId } = req.body;
            if (!(0, auth_utils_1.isValidString)(roomId, 1, 50) || !(0, auth_utils_1.isValidString)(userId, 1, 50)) {
                baseResponseInst.setValue(400, "Invalid parameters", null);
                return res.status(400).json(baseResponseInst.buildResponse());
            }
            try {
                const submission = yield submission_model_1.Submission.findOne({ roomId, userId });
                if (!submission) {
                    baseResponseInst.setValue(404, "Submission not found", null);
                    return res.status(404).json(baseResponseInst.buildResponse());
                }
